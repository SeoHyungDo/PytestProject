...
from pageobject.coupang_obj import coupang_main

class Test_CoupangTest(passclass) :

    def test_favorite_name(self) :
        Coupang_main = coupang_main(self.driver)
        favorite_name_text = Coupang_main.favorite_name_obj().text
        assert favorite_name_text == "즐겨찾기"

    def test_apply_store(self) :
        Coupang_main = coupang_main(self.driver)
        apply_store_text = Coupang_main.apply_store_obj().text
    ...
    def test_apply_store_selectbox_action(self):
        ...
        apply_store_select_box_data = self.driver.find_elements(By.XPATH,'//*[@id="subscribeHeader"]/li[2]/p')
